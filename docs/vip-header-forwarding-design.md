...
![High-Level Design](../misc/images/02-high-level-design.png)
...
### 4.1 单元测试（已完成，共 19 个，全部通过）

| 模块 | 测试文件 | 测试数 | 覆盖内容 |
|------|---------|--------|---------|
| `boom-core` | `provider.rs` | 5 | `RequestContext` 默认值为 0、VIP 常量为 100、支持任意优先级值、Clone 正确、VIP 与 Normal 互不相等 |
| `boom-provider` | `openai.rs` | 5 | VIP 请求 header 为 `X-Gateway-Priority: 100`、普通请求为 `0`、自定义值 `42`、streaming 同样注入、Authorization 与 Priority header 共存 |
| `boom-main` | `routes.rs` | 9 | `is_vip_key` 对 `true`/`false`/缺失/空对象/null/string "true"/数字 1 的判断，以及 `is_vip → priority` 映射（`true`→100, `false`→0） |

其中 `boom-provider` 的测试使用 `wiremock` mock HTTP server，验证实际发出的 HTTP 请求中 `X-Gateway-Priority` header 的值与预期一致。

### 4.2 集成验证

用 `tcpdump` / `wireshark` 抓 BoomGateway → 下游调度器流量，确认 `X-Gateway-Priority: <数值>` header 存在且值正确。

### 4.3 回归验证

灰度期间直连 vLLM 的 90% 流量，观察错误率和延迟无变化（vLLM 忽略未知 header，不受影响）。
