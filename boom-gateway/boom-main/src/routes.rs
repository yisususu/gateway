...
async fn chat_completions_inner(
    ...
    check_model_access(identity, &req.model, &state.router, &state.inner.load().config.general_settings.public_models)
        .map_err(|e| {
...
pub async fn messages(
    ...
    // 1. Model access check (deployment-aware, alias-aware).
    check_model_access(identity, &openai_req.model, &state.router, &state.inner.load().config.general_settings.public_models)
        .map_err(|e| {
